...
const DEFAULT_CACHE_CONTROL_HEADER_VALUE: &str =
    "public, max-age=300, s-maxage=900, stale-while-revalidate=300, stale-if-error=300";
...
pub async fn redirect(
    ...
    let select_timout = tokio::time::Duration::from_millis(300);
    ...
    match saved_statistic {
        ...
        _ => tracing::debug!(
            "Persisted new link click for link with id {}, referer: {:?}, user_agent: {:?}",
            requested_link,
            referer_header,
            user_agent_header
        ),
...
pub async fn create_link(
    ...
    for _ in 1..=3 {
        ...
        let new_link = tokio::time::timeout(
            ...
            sqlx::query_as!(
                ...
                select id, target_url from insert_link
                "#,
                &new_link_id,
                &url
            )
        ...
        match new_link {
            ...
            Err(err) => match err {
                Error::Database(db_err) if db_err.kind() == ErrorKind::UniqueViolation => {
                    tracing::debug!("Link with id {} already exists, retrying", new_link_id);
                }
    ...
    tracing::error!(
        "Could not persist new short link. Exausted all retries of generating a unique id."
    );
...
pub async fn get_link_statistics(
    ...
    let select_statistics_timeout = tokio::time::Duration::from_millis(300);

    let statistics = tokio::time::timeout(
        select_statistics_timeout,
        sqlx::query_as!(
            CountedLinkStatistic,
            r#"
            select count(*) as amount, referer, user_agent from link_statistics
            group by link_id, referer, user_agent having link_id = $1
            "#,
            &link_id
        )
        .fetch_all(&pool),
